...
function COCONUTCard({ result }: { result: COCONUTResult | null }) {
  ...
  return (
    <div className={`border rounded-lg p-4 ${result.found ? 'border-green-200 bg-green-50' : 'border-gray-200'}`}>
      <div className="flex items-center gap-2 mb-2">
        <span className="text-lg">🌿</span>
        <span className="font-medium text-gray-900">COCONUT</span>
        {result.found ? (
          <span className="px-2 py-0.5 bg-green-100 text-green-800 text-xs rounded-full">Natural Product</span>
        ) : (
          <span className="px-2 py-0.5 bg-gray-100 text-gray-600 text-xs rounded-full">Not Found</span>
        )}
      </div>
        ...
        <div className="mt-3 space-y-2 text-sm">
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <span className="font-mono text-green-700">{result.coconut_id}</span>
            </div>
            {result.name && (
              <div>
                <span className="text-gray-500">Name:</span>{' '}
            ...
            {result.nplikeness != null && (
              <div>
                ...
                <span className={result.nplikeness > 0 ? 'text-green-700' : 'text-gray-600'}>
