...
_nts = None
# Set once an init attempt fails; prevents re-attempting JVM startup, which can
# leave JPype's JVM in an inconsistent state on a second try.
_init_failed = False
...
def init_opsin() -> None:
    ...
    singleton. This should be called once during application startup. The first
    failed attempt re-raises (so startup can log it) and sets a sticky flag;
    subsequent calls short-circuit without re-attempting JVM startup.
    ...
    """
    global _nts, _init_failed

    if _nts is not None:
        return  # Already initialized

    if _init_failed:
        # A prior attempt failed — do not retry. OPSIN stays unavailable and
        # callers fall back to PubChem.
        return
