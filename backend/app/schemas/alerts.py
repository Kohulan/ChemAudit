...
class AlertResultSchema(BaseModel):
    ...
    matched_atoms: List[int] = Field(default_factory=list, description="Atom indices matched by the pattern")
    catalog_source: str = Field(..., description="Source catalog (PAINS, BRENK, etc.)")
    smarts: Optional[str] = Field(None, description="SMARTS pattern that matched (if available)")
...
class AlertScreenRequest(BaseModel):
    ...
    def validate_catalogs(cls, v: List[str]) -> List[str]:
        ...
        valid_catalogs = {"PAINS", "PAINS_A", "PAINS_B", "PAINS_C", "BRENK", "NIH", "ZINC", "ALL"}
        normalized = []
        for cat in v:
            cat_upper = cat.upper()
            if cat_upper not in valid_catalogs:
                raise ValueError(f"Unknown catalog: {cat}. Valid: {sorted(valid_catalogs)}")
...
class AlertScreenResponse(BaseModel):
    ...
    molecule_info: MoleculeInfoSchema = Field(..., description="Information about the screened molecule")
    alerts: List[AlertResultSchema] = Field(default_factory=list, description="List of matched alerts")
    total_alerts: int = Field(default=0, description="Total number of alerts found")
    screened_catalogs: List[str] = Field(default_factory=list, description="List of catalogs screened")
    has_critical: bool = Field(default=False, description="True if any critical alerts found")
    has_warning: bool = Field(default=False, description="True if any warning alerts found")
...
class CatalogListResponse(BaseModel):
    ...
    catalogs: Dict[str, CatalogInfoSchema] = Field(..., description="Available catalogs")
    default_catalogs: List[str] = Field(default=["PAINS"], description="Default catalogs if none specified")
