...
class StandardizationStep(BaseModel):
    ...
    changes: str = Field(default="", description="Description of changes made by this step")


class StereoComparison(BaseModel):
    """Comparison of stereochemistry before and after standardization."""

    before_count: int = Field(default=0, description="Number of defined stereocenters before")
    after_count: int = Field(default=0, description="Number of defined stereocenters after")
    ...
    double_bond_stereo_lost: int = Field(default=0, description="Number of E/Z stereo bonds lost")
    warning: Optional[str] = Field(None, description="Warning message if stereochemistry was lost")
...
class CheckerIssue(BaseModel):
    ...
    penalty_score: int = Field(..., description="Penalty score for this issue (higher = worse)")
...
class StandardizationResult(BaseModel):
    ...
    success: bool = Field(default=False, description="Whether standardization was successful")
    error_message: Optional[str] = Field(None, description="Error message if failed")

    steps_applied: List[StandardizationStep] = Field(
        default_factory=list,
        description="List of pipeline steps and their results"
    )

    checker_issues: List[CheckerIssue] = Field(
        default_factory=list,
        description="Issues found by ChEMBL structure checker"
    )
    ...
    stereo_comparison: Optional[StereoComparison] = Field(
        None,
        description="Stereochemistry comparison before/after"
    )

    structure_comparison: Optional[StructureComparisonSchema] = Field(
        None,
        description="Structure comparison before/after"
    )

    mass_change_percent: float = Field(
        default=0.0,
        description="Percentage change in molecular weight"
    )
...
class StandardizeRequest(BaseModel):
    ...
    options: StandardizationOptions = Field(
        default_factory=StandardizationOptions,
...
class StandardizeResponse(BaseModel):
    ...
    molecule_info: MoleculeInfo = Field(..., description="Information about the original molecule")
